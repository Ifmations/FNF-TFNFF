...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['you must be hacking!', 1], //From 90% to 99%
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				var stageFront:BGSprite = new BGSprite('stagefront', -650, 2000, 1, 1);
			...
			case 'limo': //Week 4
				...
				if(!ClientPrefs.lowQuality) {
					...
					GameOverSubstate.characterName = 'deaddjab-dead';
			...
			case 'campDAY': //Week 1
				var bgcamp:BGSprite = new BGSprite('campday', -600, -200, 0.9, 0.9);
				add(bgcamp);

			case 'caged': //caged
			    var bg:BGSprite = new BGSprite('caged/back', -600, -200, 0.9, 0.9);
				add(bg);

			case 'casino': //Week 1
			    var bg:BGSprite = new BGSprite('casino', -600, -200, 0.9, 0.9);
				add(bg);

			case 'road': //Week 1
				var bg:BGSprite = new BGSprite('road', -600, -200, 0.9, 0.9);
				add(bg);

			case 'finale': //Week 1
				var bg:BGSprite = new BGSprite('evilroad', -600, -200, 0.9, 0.9);
				add(bg);
			...
			case 'record':
				var bg:BGSprite = new BGSprite('record', -600, -200, 0.9, 0.9);
				add(bg);
		}
		...
		if(!modchartSprites.exists('blammedLightsBlack')) { //Creates blammed light black fade in case you didn't make your own
			...
			blammedLightsBlack.makeGraphic(Std.int(FlxG.width * 30), Std.int(FlxG.height * 30), FlxColor.BLACK);
		...
		var file:String = Paths.json(songName + '/dialogue2'); //Checks for json/Psych Engine dialogue
		if (OpenFlAssets.exists(file)) {
			dialogueJson2 = DialogueBoxPsych.parseDialogue(file);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'beatdrop' | 'deathnote' | 'oblivion':
				...
				case 'campfire' | 'left-for-dead' | 'checkmate':
				    startDialogue(dialogueJson);

				case 'science' | 'captured' | 'caged':
				    startDialogue(dialogueJson);

				case 'kept-away' | '!danger!' | 'final-loss' | 'finale':
				    startDialogue(dialogueJson);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				introAssets.set('finale', ['ready-finale', 'set-finale', 'go-finale']);
	...
	function screenshake():Void {
        FlxTween.tween(FlxG.camera, { angle: FlxG.camera.angle + 360}, 0.75, { type: FlxTween.ONESHOT , ease: FlxEase.cubeInOut});
	}

	function screenshakehalf():Void {
        FlxTween.tween(FlxG.camera, { angle: FlxG.camera.angle + 180}, 0.75, { type: FlxTween.ONESHOT , ease: FlxEase.cubeInOut});
	}
    
	function stuffshake():Void {
        FlxTween.tween(camHUD, { angle: camHUD.angle + 360}, 0.75, { type: FlxTween.ONESHOT , ease: FlxEase.cubeInOut});
	}

	function stuffshakehalf():Void {
        FlxTween.tween(camHUD, { angle: camHUD.angle + 180}, 0.75, { type: FlxTween.ONESHOT , ease: FlxEase.cubeInOut});
	}

	function stuffmove():Void {
		FlxTween.tween(camHUD, {y: camHUD.y + 600, alpha: 1}, 1, { type: FlxTween.PINGPONG , ease: FlxEase.cubeInOut});
	}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
                    if(Paths.formatToSongPath(SONG.song) == 'oblivion') //to be continued...?
						{
							cancelFadeTween();
						...
						{
							if(Paths.formatToSongPath(SONG.song) == 'oblivion') {
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								health -= 1; //1 + 1 = 1 (-100%) of HP if you hit a hurt note
								if(!note.noteSplashDisabled) {
									spawnNoteSplashOnNote(note);
								}
							}
							else health -= 1; //1 + 1 = -1 (-100%) of HP if you hit a hurt sustain note
				...
				case 'Djab Note': //djab note
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							RecalculateRating();
							if(!note.isSustainNote) {
								health -= -0.50; //1 + 1 = 1 (50%) of HP if you hit a hurt note
								if(!note.noteSplashDisabled) {
									spawnNoteSplashOnNote(note);
								}
							}
							else health -= 1; //1 + 1 = -1 (50%) of HP if you hit a hurt sustain note
				...
				case 'Event Note': //Event note
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(dad.animation.getByName('SCREE') != null) {
								dad.playAnim('SCREE', true);
							}
						}
				...
				case 'Reverse Note': //Hurt note
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							RecalculateRating();
							if(!note.isSustainNote) {
								health -= 0; //0 + 0 = 0 (-0%) of HP if you hit a hurt note
								if(!note.noteSplashDisabled) {
									spawnNoteSplashOnNote(note);
								}
							}
							else health -= 0; //0 + 0 = 0 (-0%) of HP if you hit a hurt note
	...
	override function beatHit()
	{
		...
		if (curSong == 'reset' && dad.curCharacter == 'monster' && dad.animation.curAnim.name.startsWith("sing")){
           health -= 0.040;
		}

		if (curSong == "caged" && health > 0.030) {
			health -= 0.0150;
		    if (health < 0.030) {
				health = 0.030;
			}
